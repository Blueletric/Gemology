...
public class NetworkHandler {
    ...
    public static void init(ASMDataTable asmData) {
        ...
        channels = NetworkRegistry.INSTANCE.newChannel("TG", new PacketCodec(), TechnicallyGems.proxy.getNewPacketHandler());
    ...
    public static void checkPacket(TGPacketBase packet, Side properSenderSide) {
    ...
    public static void sendToAllPlayers(TGPacketBase packet) {
    ...
    public static void sendPacketToPlayer(TGPacketBase packet, EntityPlayer player) {
        checkPacket(packet, Side.SERVER);
    ...
    public static void sendToAllAround(TGPacketBase packet, int dimension, double x, double y, double z, double range) {
        sendToAllAround(packet, new NetworkRegistry.TargetPoint(dimension, x, y, z, range));
    }

    public static void sendToAllAround(TGPacketBase packet, NetworkRegistry.TargetPoint point) {
        checkPacket(packet, Side.SERVER);
    ...
    public static void sendPacketToServer(TGPacketBase packet) {
    ...
    private static void registerPacket(String s) {
        try {
            ...
            if (TGPacketBase.class.isAssignableFrom(clazz)) {
    ...
    @XUPacket
    public static class PacketCrashLog extends TGPacketServerToClient {
        static Logger logger = LogManager.getLogger(TechnicallyGems.MODID + "_Crash");
        String string;
